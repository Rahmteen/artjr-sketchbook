...
router.post('/sketch', upload.single('file'), async (req: Request, res: Response) => {
  ...
  if (parentSketchId) {
    const parent = db.prepare('SELECT * FROM sketches WHERE id = ?').get(parentSketchId) as SketchRow | undefined;
    if (parent) {
      groupId = parent.group_id ?? parent.id;
      const maxVer = db.prepare('SELECT MAX(version) as v FROM sketches WHERE group_id = ?').get(groupId) as { v: number };
  ...
  const row = db.prepare('SELECT * FROM sketches WHERE id = ?').get(id) as SketchRow;
  ...
  createActivity('upload', 'sketch', id, { sketchTitle: row.title });
...
router.post('/sketch/replace/:id', upload.single('file'), async (req: Request, res: Response) => {
  ...
  const row = db.prepare('SELECT * FROM sketches WHERE id = ?').get(sketchId) as SketchRow;
  const notes = db.prepare('SELECT * FROM notes WHERE sketch_id = ?').all(sketchId) as NoteRow[];
  const refs = db.prepare('SELECT * FROM sketch_references WHERE sketch_id = ?').all(sketchId) as ReferenceRow[];
  createActivity('replace', 'sketch', sketchId, { sketchTitle: row.title });
...
router.post('/reference', upload.single('file'), async (req: Request, res: Response) => {
  ...
  createActivity('upload', 'reference_audio', id, { label: label || undefined });
